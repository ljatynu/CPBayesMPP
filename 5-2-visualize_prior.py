...
def visualize_prior_latent(args: Namespace):
    ...
    args.prior_latent_path = f'results/' \
                             f'{args.train_strategy}+prior/' \
                             f'{args.data_name}_checkpoints/' \
                             f'{args.data_name}_{args.visualize_type}.csv'
    args.fig_output_path = f'figures/' \
                           f'prior_latent/' \
                           f'{args.data_name}-{args.train_strategy}.jpg'
    ...
    title = f'{args.print_name[args.data_name]} ' \
            f'Uninformative Prior' if args.train_strategy == 'cd' \
...
def visualize_prior_similarity(args: Namespace):
    ...
    title = f'{args.print_name[args.data_name]} Uninformative Prior' if args.train_strategy == 'cd' else f'{args.print_name[args.data_name]} Contrastive Prior'
    plt.title(title, fontsize=20)
...
def visualize_prior(args: Namespace):
    if args.visualize_type == 'prior_similarity':
        visualize_prior_similarity(args)
    elif args.visualize_type == 'prior_latent':
