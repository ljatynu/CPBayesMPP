...
def visualize_prior_latent(args: Namespace):
    ...
    if args.dataset_type == 'classification':
        ...
        t_sne_x = np.array(t_sne_x)
        t_sne_y = np.array(t_sne_y)

        plt.scatter(t_sne_x[labels == 0], t_sne_y[labels == 0], c='purple', alpha=0.7, label='Label 0')
        plt.scatter(t_sne_x[labels == 1], t_sne_y[labels == 1], c='gold', alpha=0.7, label='Label 1')
    else:
    ...
    if args.dataset_type == 'regression':
        # Create a colorbar in the new figure
    ...
    if args.colorbar:
        ...
        fig, ax = plt.subplots(figsize=(25, 1))  # Adjust the size as needed

        # Save the colorbar figure
        colorbar.ax.tick_params(labelsize=0, labelbottom=False, labeltop=False, bottom=False, top=False)
        colorbar_fig_path = os.path.join(f'figures', f'Prior_prediction_latent',
                                         f'{args.print_name[args.data_name]}_colorbar.JPG')
        os.makedirs(os.path.dirname(colorbar_fig_path), exist_ok=True)
        fig.savefig(colorbar_fig_path, bbox_inches='tight', dpi=600)
...
if __name__ == '__main__':
    ...
    # For debugging
    # args.data_name = 'delaney'  # 'bace', 'bbbp', 'hiv'
    # args.prior = 'CPBayesMPP+Prior'  # 'BayesMPP+Prior', 'CPBayesMPP+Prior'
    # args.visualize_type = 'similarity'  # 'similarity', 'latent'
