...
def add_pretrain_args(parser: ArgumentParser):
    ...
    """
    # General arguments
    parser.add_argument('--pretrain_data_name', type=str, default='pubchem-1m-clean',
...
def add_predict_args(parser: ArgumentParser):
    ...
    :param parser: An ArgumentParser.
    """
    parser.add_argument('--data_name', type=str, default='lipo',
                        help='Downstream task name')
    parser.add_argument('--save_dir', type=str, default='results',
                        help='Directory where model checkpoints will be saved')
    parser.add_argument('--train_strategy', type=str, default='cl',
...
def modify_visualize_uncertainty_args(args: Namespace):
    ...
    if args.data_name in ['delaney', 'freesolv', 'lipo', 'qm7', 'qm8', 'pdbbind']:
        args.dataset_type = 'regression'
        args.metric = 'rmse'
        assert args.split_type == 'random'
        assert args.visualize_type == 'auco' or args.visualize_type == 'auce'

    if args.data_name in ['bbbp', 'tox21', 'hiv', 'bace', 'clintox', 'sider']:
        args.dataset_type = 'classification'
        args.metric = 'roc-auc'
        assert args.split_type == 'scaffold'
        assert args.visualize_type == 'ece'
    ...
    if args.visualize_type == 'auco':
        args.cd_path_list = [os.path.join(args.save_dir,
                                          f'cd',
                                          f'{args.split_type}_split',
                                          f'{args.data_name}_checkpoints',
                                          f'seed_{seed}',
        ...
        args.cl_path_list = [os.path.join(args.save_dir,
                                          f'cl',
                                          f'{args.split_type}_split',
                                          f'{args.data_name}_checkpoints',
                                          f'seed_{seed}',
        ...
        args.cd_path_list = [os.path.join(f'results',
                                          f'cd',
                                          f'scaffold_split',
                                          f'{args.data_name}_checkpoints',
                                          f'seed_{seed}',
        ...
        args.cl_path_list = [os.path.join(f'results',
                                          f'cl',
                                          f'scaffold_split',
                                          f'{args.data_name}_checkpoints',
                                          f'seed_{seed}',
...
def add_active_train_args(parser: ArgumentParser):
    ...
    parser.add_argument('--data_name', type=str, default='delaney',
                        help='Downstream task name')
    parser.add_argument('--train_strategy', type=str, default='cl+active',
                        choices=['cd', 'cl'],
                        help='Training strategy of downstream task.'
                             'cd means training with uninformative prior.'
                             'cl means training with contrastive prior.')
    ...
    parser.add_argument('--active_learning_type', type=str, default='explorative',
                        choices=['random', 'explorative'],
    ...
    parser.add_argument('--init_ratio', type=float, default=0.2,
    ...
    parser.add_argument('--active_train_times', type=int, default=6,
...
def add_prior_predict_args(parser: ArgumentParser):
    ...
    parser.add_argument('--train_strategy', type=str, default='cl',
                        ...
                        help='Training strategy of downstream task.'
                             'cd means training with uninformative prior.'
                             'cl means training with contrastive prior.')
    parser.add_argument('--prior_predict_type', type=str, default='prior_similarity',
                        choices=['prior_similarity', 'prior_latent'],
                        help='Methods of uncertainty calibration.'
