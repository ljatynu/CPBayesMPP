...
from utils.parsing import add_ood_train_args, modify_ood_train_args
...
def run_training(args: Namespace, logger: logging.Logger):
    ...
    train_data, val_data, test_data = split_data(data=data, split_type=args.split_type, sizes=args.split_sizes,
                                                 seed=args.seed, args=args, logger=logger, balance=False)
    ...
    if args.train_strategy == 'CPBayesMPP+OOD':
...
if __name__ == '__main__':
    ...
    add_ood_train_args(parser)
    args = parser.parse_args()

    modify_ood_train_args(args)
