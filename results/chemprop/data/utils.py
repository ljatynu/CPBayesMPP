...
def get_header(path: str) -> List[str]:
    """
    Returns the header of a dataset CSV file.
...
def get_num_tasks(path: str) -> int:
    """
    Gets the number of tasks in a dataset CSV file.
...
def get_smiles(path: str, header: bool = True) -> List[str]:
    """
    Returns the smiles strings from a dataset CSV file (assuming the first line is a header).
...
def get_data(path: str,
    ...
    :param max_data_size: The maximum number of dataset points to load.
    :param use_compound_names: Whether file has compound names in addition to smiles strings.
    :param logger: Logger.
    ...
    with open(path) as f:
        ...
        # Generate molecule dataset using lines data, lines = [[smiles0, target0],[smiles1, target1],...,[smilesN, targetN]]
        # Transfer lines into [[MoleculeDatapoint0],[MoleculeDatapoint1],...,[MoleculeDatapointN]]
...
def split_data(data: MoleculeDataset,
    ...
    :param sizes: A length-3 tuple with the proportions of dataset in the
    train, validation, and test sets.
    :param seed: The random seed to use before shuffling dataset.
    :param args: Namespace of arguments.
    :param logger: A logger.
    :return: A tuple containing the train, validation, and test splits of the dataset.
    """
    ...
    if split_type == 'crossval':
        ...
        if not val_fold_index:
            assert sizes[2] == 0  # test set is created separately so use all of the other dataset for train and val
        ...
        # assert len(dataset) == sum([len(fold_indices) for fold_indices in all_fold_indices])
        ...
        return scaffold_split(data, sizes=sizes, balanced=balance, seed=seed, logger=logger)
...
def validate_data(data_path: str) -> Set[str]:
    """
    Validates a dataset CSV file, returning a set of errors.

    :param data_path: Path to a dataset CSV file.
